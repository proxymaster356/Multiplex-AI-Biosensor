...
```
biohackathon/
├── run.py    run2.py                          ← Single command to run everything
├── pipeline/
│   ├── signal_processor.py            ← Layer 2: Kalman + features
│   ├── multimodal_encoder.py          ← Layer 3a/3b/3c: CNN + Attention + MC Dropout
│   ├── models.py                      ← Layer 3d: XGBoost/GB/RF task heads
│   ├── clinical_report.py             ← Layer 4+5: HTML report generator
│   └── train_pipeline.py              ← (standalone eval script)
├── biosensor_full_results.csv         ← All 100 test predictions
└── reports/
    ├── report_BIO0206.html
    ├── report_BIO0426.html
    └── ... (10 HTML reports total)
```
